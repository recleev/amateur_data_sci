...
From this we can already say that $\frac{1}{2}a \le x \le a$. If $x \le \frac{1}{2}a$, then we will get an imaginary side.
...
$$
\begin{equation}
A_{\triangle} = \frac{1}{2} \lvert \sum_{i = 1}^n x_i (y_{i + 1} - y_{i - 1}) \rvert \tag{Shoelace Formula} \label{shoelace}
\end{equation}
$$
...
Applying $\eqref{shoelace}$ to a triangle,
...
$$
\begin{align}
A_{\triangle} &= \frac{1}{2} \lvert a(y - 0) + x\left[0 - \left(y - \sqrt{2ax - a^2}\right)\right] + 0 \cdot \left(y - \sqrt{2ax - a^2} - y\right)
\\
\\
&= \frac{1}{2} \lvert ay  - x\left(y - \sqrt{2ax - a^2}\right)\rvert
\\
\\
A_{\triangle} &= \frac{1}{2}ay - \frac{1}{2}xy + \frac{1}{2} x \sqrt{2ax - a^2} \tag{Area Eqn 1} \label{area1}
\end{align}
$$
...
$$
A_{\triangle} = \frac{1}{2}xy \tag{Area Eqn 2} \label{area2}
$$

This also gives us $y$ as a function of $x$ and the $A_{\triangle}$, 

$$
y = 2A_{\triangle}/x \tag{a} \label{y1}
$$

Substituting $\eqref{area2}$ and $\eqref{y1}$ in $\eqref{area1}$,
...
\\
\\
A_{\triangle} &= \frac{x^2 \sqrt{2ax - a^2}}{4x - 2a} = x^2 \left( 2ax - a^2 \right)^{1/2} \left( 4x -2a \right)^{\text -1} \tag{Area Eqn 3} \label{area3}
\end{align}
$$

I am showing $\eqref{area3}$ in alternate form, a product of terms rather than a quotient, as this is easier to differentiate.

$$
\begin{align}
\frac{d}{dx}A_{\triangle} &= 2x \left(2ax - a^2\right)^{1/2} \left(4x - 2a\right)^{\text -1} + x^2 \left( \frac{1}{2}  \right) \left( 2a \right)\left(2ax - a^2\right)^{\text -1/2} \left(4x - 2a\right)^{\text -1} + x^2 \left(2ax - a^2\right)^{1/2} \left(-1 \right) \left(4x - 2a\right)^{\text -2}
\\
\\
&= \frac{2x \left(2a - a^2\right)^{1/2}}{4x - 2a} + \frac{a x^2}{\left(2ax - a^2\right)^{1/2} \left(4x - 2a\right)} - \frac {x^2 \left(2ax - a^2\right)^{1/2}}{\left(4x - 2a\right)^2}
\\
\\
&= \frac{2x \left(2x - a^2\right) \left(4x - 2a\right) + ax^2 \left( 4x - 2a \right) - x^2 \left( 2ax - a^2\right)}{\left(2ax - a^2\right)^{1/2}\left(4x - 2a\right)^2}
\end{align}
$$
