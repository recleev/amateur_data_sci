...
We know that $x = \frac{1}{2}a$ is not a possible solution, so the the minimum area is achieved when $x = \frac{2}{3}a$.
...
\\
\\
l_{AC} &= \left( 2ax^3 \right)^{\text 1/2} \left( 2ax -a^2 \right)^{\text -1/2} \tag{Crease Eqn} \label{crease}
\end{align}
$$
...
All of the above steps are the traditional way of finding the extremas of a value as a function of another value. We can solve the same problems using only $\eqref{area3}$ and $\eqref{crease}$ without differentiation.
