...
The randomness of stock prices are usually enough to push away people from investing in the stock market. Monitoring prices can be very stressfull when you see the market and the stocks in your protfolio all going down.

There are different opinions and strategies on how invest in stocks and to ensure low risk and decent rewards. My favorite strategy is the *buy and hold*, buy a stock, hold it for the long run. I like this strategy because it is less stressfull than watching the prices everyday then buying/selling at opportune moments.

Another strategy I like with stocks is regular buying of stocks at fixed intervals with a fixed budget everytime. For example, we can promise ourselves that we will invest ₱6,000 every six months on the stock market. When the price of our portfolio is up, we buy less. When the price of our portfolio is down, we buy more. Then we hold.

Buy and hold is not a sure way to win in the stock market. If the market goes down, and stays down for 100 years, then buy and hold will fail for the investor who wants to benefit from the investment. However, I think this is the simplest, easiest, and less stressfull strategy one can adopt.
...
The simulation is as follows: Starting `r start_date`, the hypothetical investor uses ₱6,000 to buy PSEi stocks every six months until `r end_date`. The hypothetical investor will accumulate a `r (6000 * nrow(invest_psei_simulation)) %>% peso()` over this period.
...
Impatience and early panicking cost. If one panicked and gaved up with PSEi after experiencing a net decay rate of `r min_rate %>% percent()` at the end of `r min_rate_year` then one would have missed the `r max_rate %>% percent()` at the end of `r max_rate_year`. However, buying out at the maximum rate would only gain our hypothetical investor `r filter(annual_interest_rate_estimated, avg_interest_rate == max_rate) %>% mutate(gain = buy_hold - cumulative_cost) %>% pull(gain) %>% peso()` and miss the `r filter(annual_interest_rate_estimated, n_years == max(n_years)) %>% mutate(gain = buy_hold - cumulative_cost) %>% pull(gain) %>% peso()` 
