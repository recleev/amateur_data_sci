...
Sentiment analysis is another staple when working with text data. Words express emotions. Some express more negative emotions, others more positive. 
...
words %>% 
  ...
  group_by(sentiment) %>% 
  top_n(10, percent_sentiment) %>% 
  ungroup() %>%  
  ggplot(aes(reorder_within(chapter, 
                            percent_sentiment,
                            sentiment),
             percent_sentiment,
             fill = sentiment)) +
