...
categories: ["text mining", "tidyverse"]
tags: ["frankenstein", "literature", "sentiment analysis"]
output:
  blogdown::html_page:
    toc: true
---
...
frankenstein %>% 
  head(20) %>% 
  kable(caption = "Sample Raw Downloaded Frankenstein Text from Guntenberg")
...
words %>% 
  ...
  labs(title = "Number of Words per Chapter",
       subtitle = sprintf("Red Line is the Median Number of Words (%s) per Chapter",
                          comma(n_words_median_chapter)),
       x = "Chapter",
       y = "Number of Words") +
  theme_pander()
...
words %>% 
  ...
  labs(title = "Net AFINN Score per Chapter of Frankenstein",
       x = "Chapter",
...
tibble(chapter_title = c("Letter 1 - Robert Walton", 
                         ...
                         "Chaper 15 - Paradise Lost, Plutarch's Lives and Sorrows of Werter",
  ...
  separate(chapter_title,
           into = c("chapter",
                    "title"),
           sep = " - ") %>% 
  kable(caption = "My Titles for Each Frankenstein Chapter")
...
words %>% 
  ...
  theme_pander() +
  labs(title = "Cumulative AFINN Score Through Frankenstein",
...
words %>% 
  ...
  scale_fill_pander() +
  theme_pander() +
  labs(title = "Frequency of the Eight Emotions in Frankenstein",
       x = "Emotions",
       y = "")
...
Alas, this tidytext analysis fails if I have not sparked any interest in you to read Frankenstein. Mary Wollstonecraft Shelley's masterpiece must continue to burn inside the hearts and minds of the people. It might be only way to caution the Victor and appease the Daemon inside us. 
