...
$$
\begin{equation}
F = \frac{d \left( mv \right)}{dt} \tag{1} \label{momentum-rate}
\end{equation}
$$

in terms of the momentum $mv$ of a particle of mass $m$ and velocity $v$, and remains valid even if $m$ is not constant. A spherical raindrop falls through air with water vapor, and assume that by condensation the mass of the raindrop increases at a rate proportional to its surface area, with $c$ the constant of proportionality. If the intial radius and velocity of the raindrop are both zero, show that the drag exerted bt the condensation of the water vapor has the effect of making the raindrop fall with acceleration $\frac{g}{4}$.
...
\begin{align}
\begin{split}
\frac{d}{dr} \left( r^3 v \right) &= \left( \frac{\delta}{c} \right) r^3 g
\\
\\
...
$$
\begin{equation}
\frac{dm}{dt} = 4 \pi r^2 c \tag{3} \label{mass-rate}
\end{equation}
$$

$\eqref{hint}$ only confused me at the beginning, but it gave me clues later on how to solve the problem. The first clue was the presence of the raindrop's density $\delta$.

Mass is the product of volume and density. Since the raindrop is assumed to be spherical,

$$
\begin{equation}
m = \frac{4}{3} \pi r^3 \delta \tag{4} \label{volume-density}
\end{equation}
$$

Keeping in mind that $m$, $r$, and $v$ all vary with time and all other values are constant, I expanded (by the [product rule](https://amateurdatasci.rbind.io/blog/deriving-quotient-rule-product-rule/)) $\eqref{momentum-rate}$ both from itself and by using $\eqref{volume-dentsity}$, i.e.
...
Substituting $\eqref{mass-rate}$ and $\eqref{volume-density}$,
...
\\
\\
dt &= \frac{\delta}{c} dr \tag{5} \label{radius-time}
\end{align}
$$

Another thing to remember, the force acting on the raindrop, despite it's variable size, is always equal to it's weight,
...
\\
\\
\frac{d}{dr} \left( r^3 v \right) &= \frac{\delta}{c} r^3 g \tag{6} \label{hint-proof}
\end{align}
$$

This is proving $\eqref{hint}$ and also sets up to integrate in terms of $r$.
...
\\
\\
v &= \frac{\delta g r}{4c} \tag{7} \label{velocity-radius}
\end{align}
$$

Differentiating $\eqref{velocity-radius}$ with respect to time,
...
However, from $\eqref{radius-time}$, $\cfrac{dr}{dt} = \cfrac{c}{\delta}$
