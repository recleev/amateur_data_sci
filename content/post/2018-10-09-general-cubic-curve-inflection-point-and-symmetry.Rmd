...
The abscissa of $I$ is $\frac{-b}{3a}$, which is also \@ref(eq:average-min-max), the average of $x_{min}$ and $x_{max}$.
...
\end{align} (\#eq:XY-to-expand)
$$

Before I proceed, I think this is a good opportunity to review two, in my opinion, must-know binomial expansion
...
Let's review a curve's symmetry. A curve defined by $f \left( x \right)$ is symmetric with respect to the $x$-axis if $f \left( x \right) = - f \left( x \right)$, with respect to the $y$-axis if $f \left( x \right) = f \left( -x \right)$, with respect to the origin if $f \left( x \right) = - f \left( -x \right)$. Thus, we can prove that \@ref(eq:XY-cubic) is symmetric to its origin, if $Y = F \left( X \right) = - F \left( -X \right)$.
...
```{r plot-general-cubic, echo=TRUE,fig.align='center', fig.cap="**The Generic Cubic Curve**. The point of inflection is colored green, the minimum point blue, and the maximum point red."}
