...
\\
\\
\cos \left( 2 \theta \right) &= \cos^2 \theta - \sin^2 \theta (\#eq:double-angle-sine)
\end{align}
$$
...
Problem 62, page 62 in @cwag2016.
...
\\
\\
\sin m \theta \cos n \theta &= \frac{1}{2} \left[ \sin \left( m + n \right) \theta - \sin \left( m - n \right) \theta \right] (\#eq:problem-sine-sine) (\#eq:problem-sine-cosine)
\\
\\
...
Problems \@ref(eq:problem-sine-sine), \@ref(eq:problem-sine-cosine), and \@ref(eq:problem-cosine-cosine) already offer clues on the proofs' process. The left hand side of the equations can be seen in identities \@ref(eq:sum-sine) and \@ref(eq:sum-cosine). We will also need identities \@ref(eq:negative-sine) and \@ref(eq:negative-cosine) to simplify the sine and cosine of the negative angles.
...
We derive \@ref(eq:problem-sine-sine) by subtracting \@ref(eq:cosine-m-minus-n) from \@ref(cosine-m-plus-n).
...
Problem \@ref(eq:problem-sine-cosine) by adding \@ref(sine-m-plus-n) and \@ref(sine-m-minus-n).
