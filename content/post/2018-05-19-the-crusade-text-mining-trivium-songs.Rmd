...
tags: ["trivium", "heavy metal", "lyrics", "geniusR"]
output:
  blogdown::html_page:
    toc: true
...
```{r most-words-per-album, fig.height=5,fig.width=8, fig.cap="Number of Words in Each Trivium Album"}
clean_trivium_lyrics %>% 
  count(album) %>% 
  ggplot(aes(album,
             n)) +
...
```{r most-unique-words-per-album, fig.height=5,fig.width=8, fig.cap="Number of Unique Words in Each Trivium Album"}
...
```{r common-words-per-album, fig.cap="Most Common Words in Every Trivium Album"}
...
```{r lyrics-tfidf, fig.cap="TF-IDF of Words in Every Trivium Album"}
...
```{r proportion-positive-negative, fig.cap="Proportion of Positive and Negative Words in Every Trivium Album. Based on Bing Liu's Sentiment Lexicon"}
...
```{r afinn-scores-album, fig.cap="Total Negative AFINN Scores of Trivium Albums"}
...
```{r mean-afinn-scores-album, fig.cap="Negative Mean AFINN Scores of Trivium Albums"}
...
```{r afinn-negative-trivium-songs, fig.cap="Top Five Most Negative Trivum Songs per Album. Based on AFINN Scores"}
...
```{r trivium-lyrics-wordcloud, fig.cap="Most Frequent Words in Trivum Songs"}
